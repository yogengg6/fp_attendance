 ...
 */
int MDLDB_Connector::dbconnect(const char * const db_host,
                                    const char * const db_user,
                                    const char * const db_passwd
                                    )
                                    throw(MDLDB_Exception)
{
    try {
        ...
        if (e.getErrorCode() == 10061) {
            ...
            throw MDLDB_Exception("connect to fail", MDLDB_CONNECTION_FAIL);
        } else if (e.getErrorCode() == 1045) {
            this->status = MDLDB_CONNECTION_REFUSED;
            throw MDLDB_Exception("connect to fail", MDLDB_CONNECTION_REFUSED);
        }
    }
    return this->status;
}
 ...
 */
int MDLDB_Connector::associate_course(const string course_name)
{
 ...
 */
int MDLDB_Connector::associate_session(const string session_name)
{
    time_t now = time(NULL);
    sql::PreparedStatement *prep_stmt = NULL;
    sql::ResultSet *rs = NULL;
    static const char* prep_sql = "SELECT id FROM mdl_attendance_sessions WHERE description=? AND sessdate BETWEEN ? AND ?";
    try {
